    ...
    <div className="w-full h-full flex flex-col p-6 text-white bg-[#0a0a0c]">
      ...
      <div className="flex items-center justify-between pb-6 border-b border-white/10 mb-6">
        ...
        {user ? (
          ...
          <div className="flex items-center gap-3">
            <span className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-amber-500/10 text-amber-400 text-xs font-semibold border border-amber-500/20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="p-6 rounded-2xl bg-white/[0.03] border border-white/10 hover:border-indigo-500/30 transition-all">
          ...
          <div className={`text-[11px] font-mono px-3 py-1.5 rounded-lg inline-block ${
            user ? 'text-emerald-400 bg-emerald-500/10 border border-emerald-500/20' : 'text-indigo-400 bg-indigo-500/10'
          }`}>
            {user ? '🟢 Voice & Speech Active' : 'Always-on listening'}
          </div>
        </div>
        ...
        <div className="p-6 rounded-2xl bg-white/[0.03] border border-white/10 hover:border-cyan-500/30 transition-all">
          ...
          <div className={`text-[11px] font-mono px-3 py-1.5 rounded-lg inline-block ${
            ...
            {user ? '🟢 Writing Style Trained' : 'GPT-4o Trained'}
          </div>
        </div>
        ...
        <div className="p-6 rounded-2xl bg-white/[0.03] border border-white/10 hover:border-purple-500/30 transition-all">
          ...
          <div className={`text-[11px] font-mono px-3 py-1.5 rounded-lg inline-block ${
            ...
            {user ? '🟢 Rules Engaged' : 'Auto Rules Active'}
          </div>
        </div>
      </div>

      {/* Gmail Auth Action Section */}
      <div className={`p-8 rounded-2xl border flex flex-col md:flex-row items-center justify-between gap-6 transition-all ${
        user
          ...
          {user ? (
            <>
              ...
              <p className="text-xs text-slate-300 max-w-xl">
                Your Gmail inbox and Google Calendar are actively synced for <strong className="text-white font-mono">{user.email}</strong>.
              </p>
            </>
          ) : (
            <>
        ...
        {user ? (
          ...
          <a
            href="/auth/google"
            className="px-6 py-3 rounded-xl bg-gradient-to-r from-indigo-500 to-cyan-500 text-white font-bold text-sm shadow-[0_0_25px_rgba(99,102,241,0.4)] hover:scale-105 transition-all text-center whitespace-nowrap flex items-center gap-2"
          >
