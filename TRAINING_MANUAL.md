...
## 9. 🐳 Server Maintenance, Log Rotation & Memory Optimization (Docker)
Since this project runs inside a Docker container (managed by Easypanel) on low-resource environments (like a 2GB RAM / 1 vCPU droplet), logs and build caches can accumulate on the host machine disk over time, and Next.js compilation can occasionally crash (`Killed`) or hang due to memory constraints.

Follow these system administration best practices to keep deployments running fast and reliably.

### 🧹 Quick Clean: Free Up Space Instantly (Logs)
To instantly clear all existing container logs and free up disk space on the droplet, SSH into the server and run (if you are already logged in as `root`, you do not need `sudo`):
```bash
truncate -s 0 /var/lib/docker/containers/*/*-json.log
```

### 🔄 Permanent Fix: Automatic Log Rotation
To limit the log size automatically so the disk never fills up again:
...
---
*Generated by Datalazo Intelligence Agency | v3.7 — The Infrastructure & Server Admin Release*
